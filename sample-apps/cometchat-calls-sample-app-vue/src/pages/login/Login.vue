...
const uidInput = ref("cometchat-uid-1");
...
watch(
  () => appStore.user,
  (user) => {
    if (user) router.push("/join-session");
  },
  { immediate: true },
);
...
const handleLogin = () => {
  ...
  CometChatCalls.login(uidInput.value).then((user) => {
    appStore.setUser({ uid: user.uid, name: user.name, avatar: user.avatar });
  });
};
        ...
        <div class="users-grid">
          ...
          >
            <div
              ...
              :class="[
                'profile-card',
                { 'profile-card-selected': uidInput === user.uid },
              ]"
              role="button"
              tabindex="0"
              :aria-pressed="uidInput === user.uid"
              @click="uidInput = user.uid"
              @keydown.enter.space.prevent="uidInput = user.uid"
            >
              <div
                v-if="uidInput === user.uid"
                class="check-badge"
                aria-hidden="true"
              >
      ...
      <div class="bottom-section">
        ...
        <div class="button-section">
          ...
          <p
            ...
            @keydown.enter.space.prevent="router.push('/credentials')"
          >
