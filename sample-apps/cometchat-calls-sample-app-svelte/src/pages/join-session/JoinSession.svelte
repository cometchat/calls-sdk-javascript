<script lang="ts">
  ...
  let inMeeting = $state(false);
  let menuRef: HTMLDivElement | undefined = $state();
  let meetingContainerRef: HTMLDivElement | undefined = $state();
  ...
  $effect(() => {
    if (inMeeting && sessionId) {
      CometChatCalls.generateToken(sessionId).then(({ token }: any) => {
        console.log('token', token);
        if (meetingContainerRef) {
          CometChatCalls.joinSession(token, {}, meetingContainerRef);
        }
      });
    }
  });
...
<div class="meeting-container" class:meeting-visible={inMeeting} bind:this={meetingContainerRef}></div>

<div class="container" class:container-hidden={inMeeting}>
  <div class="avatar-wrapper" bind:this={menuRef}>
    <button
      class="avatar-button"
      type="button"
      ...
      aria-haspopup="true"
    >
      <Avatar name={appStore.user?.name ?? ''} url={appStore.user?.avatar} size={45} />
    </button>

    {#if menuOpen}
      <div class="popup-menu" role="menu">
        <div class="popup-user-info">
          <Avatar name={appStore.user?.name ?? ''} url={appStore.user?.avatar} size={32} />
    ...
    <div class="form-content">
      <div class="fields">
        <div class="field-group">
          <label class="field-label" for="session-id-input">Enter Session Id</label>
      ...
      <button
        class="join-button {sessionId ? 'join-button-active' : ''}"
        type="button"
        disabled={!sessionId}
        onclick={() => { inMeeting = true; }}
      >
